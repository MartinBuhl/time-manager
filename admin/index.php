...
<title><?= h(t('admin.title')) ?> – Time Manager</title>
...
<div class="admin-page">
    ...
    <div class="admin-header">
        <h1><?= h(t('admin.title')) ?></h1>
        <a href="../index.php" class="btn-logout"><?= h(t('admin.toApp')) ?></a>
    </div>

    <div style="padding: 0 0 24px">

        <div class="zone-label"><?= h(t('admin.favorites')) ?></div>
        <div id="zone-top" class="admin-zone"></div>

        <div class="zone-divider"><?= h(t('admin.more')) ?></div>
...
<div id="infoModal">
    <div style="background:#fff;border-radius:10px;width:100%;max-width:440px;
        ...
        <div style="background:#1e293b;color:#fff;padding:20px 24px;display:flex;
            ...
            <span style="font-weight:700;font-size:15px"><?= h(t('admin.infoTitle')) ?></span>
        ...
        <div style="padding:24px">
            <div style="display:flex;gap:12px;margin-bottom:20px">
                <div style="flex:1;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:12px 16px">
                    <div style="font-size:11px;color:#6b7280;margin-bottom:2px"><?= h(t('admin.installedVersion')) ?></div>
                    <div style="font-size:20px;font-weight:700;color:#1e293b"><?= h(APP_VERSION) ?></div>
                </div>
                <div style="flex:1;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:12px 16px">
                    <div style="font-size:11px;color:#6b7280;margin-bottom:2px"><?= h(t('admin.available')) ?></div>
...
const CARDS = {
    customers: {
        label: t('admin.card.customers'), href: 'customers.php',
        icon: '<path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/>',
    },
    billing: {
        label: t('admin.card.billing'), href: 'billing.php',
    ...
    invoices: {
        label: t('admin.card.invoices'), href: 'invoices.php',
    ...
    entries: {
        label: t('admin.card.entries'), href: 'entries.php',
    ...
    orders: {
        label: t('admin.card.orders'), href: 'orders.php',
    ...
    expenses: {
        label: t('admin.card.expenses'), href: 'expenses.php',
        icon: '<path d="M15 18.5c-2.51 0-4.68-1.42-5.76-3.5H15v-2H8.58c-.05-.33-.08-.66-.08-1s.03-.67.08-1H15V9H9.24C10.32 6.92 12.5 5.5 15 5.5c1.61 0 3.09.59 4.23 1.57L21 5.3C19.41 3.87 17.3 3 15 3c-3.92 0-7.24 2.51-8.48 6H3v2h3.06c-.04.33-.06.66-.06 1s.02.67.06 1H3v2h3.52c1.24 3.49 4.56 6 8.48 6 2.31 0 4.41-.87 6-2.3l-1.78-1.77c-1.13.98-2.6 1.57-4.22 1.57z"/>',
    },
    payments: {
        label: t('admin.card.payments'), href: 'payments.php',
    ...
    users: {
        label: t('admin.card.users'), href: 'users.php',
    ...
    config: {
        label: t('admin.card.config'), href: 'config.php',
    ...
    backup: {
        label: t('admin.card.backup'), href: 'backup.php',
    ...
    info: {
        label: t('admin.card.info'), href: null, onclick: 'openInfoModal()',
...
const DEFAULT_LAYOUT = {
    ...
    bottom: ['activities', 'bookmarks', 'expenses', 'payments', 'trash', 'users', 'config', 'statistics', 'mailspool', 'logs', 'backup', 'info'],
};
// ----------------------------------------------------------------
...
async function checkVersion() {
    ...
    document.getElementById('infoStatus').textContent = t('admin.checkingUpdates');
    ...
    try {
        ...
        if (d.has_update) {
            document.getElementById('infoStatus').innerHTML =
                '<span style="color:#854d0e;font-weight:600">&#x2191; ' + t('admin.updateAvailable') + '</span>';
            document.getElementById('infoActions').innerHTML =
                ...
                '&#x2191; ' + t('admin.updateNow', { v: d.latest }) + '</a>';
        } else {
            document.getElementById('infoStatus').innerHTML =
                '<span style="color:#15803d;font-weight:600">&#x2713; ' + t('admin.upToDate') + '</span>';
        }
    } catch(e) {
        document.getElementById('infoLatest').textContent = '–';
        document.getElementById('infoStatus').innerHTML =
